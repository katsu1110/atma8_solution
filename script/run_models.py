...
from predictor import get_oof_ypred
from utils import validation_score
...
class RunModel(object):
    ...
    :cfg: config variable from hydra
    ...
    model = RunModel(train_df, test_df, target, features, categoricals=categoricals,
             group=None, model="lgb", cfg=cfg, target_encoding=False, seed=1220, scaler=None)
    ...
    def __init__(self, train_df : pd.DataFrame, test_df : pd.DataFrame, target : str, features : List, categoricals: List=[],
                group : str=None, model : str="lgb", task="regression", cv_method="KFold", n_splits=5, cfg={}, target_encoding=False, seed : int=1220, scaler : str=None, verbose=True):
    ...
    def calc_metric(self, y_true, y_pred): 
        ...
        if self.task == "multiclass":
            ...
            return metrics.roc_auc_score(y_true, y_pred) # log_loss
        
        elif self.task == "regression":
            return validation_score(y_true, y_pred)
    ...
    def fit(self):
        ...
        if self.scaler is not None:
            # to normal
            pt = preprocessing.QuantileTransformer(n_quantiles=100, random_state=self.seed, output_distribution="normal")
        ...
        for fold, (train_idx, val_idx) in enumerate(cv):
            ...
            if self.task == "multiclass":
                ...
                cv_score = self.calc_metric(y_vals[val_idx], np.argmax(oof_pred[val_idx, :], axis=1))
            else:
                oof_pred[val_idx] = oofs.reshape(oof_pred[val_idx].shape)
                cv_score = self.calc_metric(y_vals[val_idx], oof_pred[val_idx])
            print(f'Fold {fold} cv score: ', cv_score)
        ...
        if self.verbose:
            print('Overall CV score: ', loss_score)
